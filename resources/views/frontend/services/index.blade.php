...
<section id="hero" style="position: relative; background: url('{{ asset('frontend')}}/images/baby.png') no-repeat center center / cover; height: 300px; color: white;">
  ...
  <div class="container text-center position-relative" style="z-index: 2;">
    <h1 class="display-4">{{ $service->name_tr }}</h1>
  </div>
</section>
<div class="container my-5">
        ...
        <h3 class="text-center mt-4">{{ $service->name_tr }}</h3>
        <p class="mt-4">{!! $service->description_tr !!}</p>

        @if($subservices->count())
        <h3 class="mt-5">Alt Hizmetler</h3>
        <div class="row mt-4">
            @foreach ($subservices as $subservice)
                <div class="col-md-4">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <h5 class="card-title">{{ $subservice->definition_tr }}</h5>
                            <a href="{{ route('subservices', $subservice->slug) }}" class="btn btn-secondary">Detaylı Bilgi</a>
                        </div>
                    </div>
                </div>
            @endforeach
